...
public class H1Fix extends H1 {
    ...
    public float computeEstimate(State gs) {
        ...
        if (computeEstimate == Float.MAX_VALUE) return Float.MAX_VALUE;
        if (hmrpEstimate) {
            fixPlan(gs);
    ...
    private float fixPlan(State gs) {
        ...
        final DirectedAcyclicGraph DG = new DirectedAcyclicGraph(
            FastutilFastLookupIntVertexGSS.class
        );
            ...
            >) getActivatingConditions(
                ...
                if (terminal instanceof Comparison) {
                    ...
                    for (int b : descendants[a]) {
                        ...
                        if (
                            numericContribution1 != 0f ||
                            numericContribution1 != Float.MAX_VALUE
                        ) {
                            cum +=
                                numericContribution1 * repetitionToConsider[b];
                        }
                    }
                    if (a != cp.goal() && repetitionToConsider[a] > 1) {
                        ...
                        if (
                            ...
                            cum +=
                                numericContribution1 *
                                (repetitionToConsider[a] - 1);
                        }
                    }
                    ...
                    if (T < 0) {
                        if (saferVersion) {
                            if (false) {
                                IntArraySet achieverSet =
                                    getAllAchievers()[conditionId];
                                if (
                                    achieverSet != null &&
                                    !achieverSet.isEmpty()
                                ) {
                            ...
                            if (achiever != -1) {
                                //this is the case where there is an achiever already in the plan
                                int repetition = (int) ceil(
                                    -1f * (T / numericContribution(achiever, c))
                                );
                                if (
                                    repetitionsInThePlan[achiever].size() == 1
                                ) {
                                    repetitionsInThePlan[achiever] =
                                        new IntArraySet(
                                            Collections.singleton(
                                                maxNumRepetition[achiever] +
                                                    repetition
                                            )
                                        );
                                }
                                maxNumRepetition[achiever] =
                                    maxNumRepetition[achiever] + repetition;
                            } else {
                                if (superFix) {
                                    //mmmmmmm
                                    IntArraySet achieverSet =
    ...
    private IntArraySet computeDescendants(
        int actionId,
        IntArraySet[] prec,
