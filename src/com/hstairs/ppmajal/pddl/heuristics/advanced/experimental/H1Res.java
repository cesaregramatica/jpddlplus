...
public class H1Res extends H1 {
                    ...
                    for (var a : getAchievers(((Terminal) c).getId())) {
                        if (getActionHCost()[a] != Float.MAX_VALUE) {
                            ...
                            if (allSeen.get(a)) {
                                ...
                                localCost[a][((Terminal) c).getId()] < min
                            ) {
    ...
    class Comp implements Comparator<Pair<Condition, Float>> {
        ...
        @Override
        public int compare(
            Pair<Condition, Float> t,
            Pair<Condition, Float> t1
        ) {
    ...
    protected float estimateCost(Condition c, float previous) {
        if (c instanceof AndCond) {
            ...
            if (max == 0 || (previous <= max)) return max;
            boolean first = true;
            // System.out.println("-----------------");
                ...
                Arrays.sort(
                    ...
                    new Comparator() {
                        ...
                        public int compare(Object t, Object t1) {
                            ...
                            // return first - second;
                            return (int) (estimateCost(
                                    (Condition) t1,
                                    previous
                                ) -
                                estimateCost((Condition) t, previous));
            ...
            for (int i = 0; i < toIterate.length; i++) {
                ...
                if (i != best && !getConditionInit()[((Terminal) v).getId()]) {
                    if (first) {
                        ...
                        allSeen = new BitSet();
                        allSeen.or(
                            getDep(((Terminal) toIterate[best]).getId())
                        );
                        first = false;
                    } else {
                        // System.out.println("Updating seen of: "+(Condition) sons[prev]);
                        allSeen.or(
                            getDep(((Terminal) toIterate[prev]).getId())
                        );
                    }
                    prev = i;
            ...
            throw new UnsupportedOperationException(
                "Condition " + c.getClass() + " not supported"
            );
        }
    }
